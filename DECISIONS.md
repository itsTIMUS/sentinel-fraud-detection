...
1. **Score** — input transaction, see decision + reason codes + latency + cost breakdown
2. **Review Queue** — decisions from audit ledger, filterable by decision type
3. **Cost Analysis** — interactive break-even curve with sliders for chargeback fee, margin, friction, challenge params
4. **Model Deep Dive** — PR-AUC, precision, recall, Brier, model progression chart, dual-dataset comparison
5. **Thompson Sampling** — convergence charts showing parameters learning from outcomes
...
### What we tried
Merged `train_identity.csv` (device type, browser, OS, screen resolution) with transaction data. Added 18 new features: device (is_mobile, has_device_info), browser (is_chrome, is_safari, is_firefox, is_edge), OS (is_windows, is_ios, is_android, is_mac), screen resolution presence, UID construction (card1 + addr1 hash), additional Vesta features (V258, V201, V246, V315, V294), email match (P_emaildomain == R_emaildomain). Total features: 48 (up from 30 in v1).
...
1. C5 (762K) — counting feature
2. C1 (582K) — counting feature
3. card1 (407K) — card attribute
4. C14 (387K) — counting feature
5. V294 (383K) — Vesta engineered feature
6. uid_hash (372K) — our entity construction
