...
with st.sidebar:
    ...
    try:
        ...
        cols[0].markdown(f"**Velocity:** {'✅' if ready.get('velocity_store') else '⚠️'}")
...
with tab1:
    ...
    model_choice = st.radio("Select Model:", ["Sparkov (explainable)", "IEEE-CIS (real-world)"], horizontal=True)
    ...
    with col_result:
        if score_btn:
            # Build payload based on model choice
            if model_choice == "Sparkov (explainable)":
                payload = {
                ...
                score_url = f"{API_URL}/v1/score"
            else:
                payload = {
                    "TransactionAmt": float(amt),
                    "ProductCD": "W",
                    "card1": int(cc_num) % 20000,
                    "card2": 111, "card3": 150, "card5": 226,
                    "addr1": 315, "addr2": 87, "dist1": 19,
                    "P_emaildomain": "gmail.com", "R_emaildomain": "gmail.com",
                    "C1": 1, "C2": 1, "C5": 0, "C6": 1, "C13": 25, "C14": 1,
                    "D1": 14, "D4": 0, "D10": 0, "D15": 0,
                    "TransactionDT": int(unix_time),
                }
                score_url = f"{API_URL}/v1/score/ieee"
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    # Model badge
                    model_label = data.get("model_version", data.get("dataset", "unknown"))
                    st.caption(f"Scored by: **{model_label}**")
                    ...
                    # Reason codes (only for Sparkov — IEEE doesn't have them yet)
...
with tab2:
    ...
    try:
        ...
        if len(reviews) > 0:
            ...
            st.dataframe(reviews, use_container_width=True, height=400)
        else:
...
with tab3:
    ...
    with col_chart:
        ...
        fig.update_layout(
            title="Break-Even Curve: Where ALLOW Meets BLOCK<br><sub>Move sliders to shift thresholds</sub>",
    ...
    try:
        ...
        with col_l:
            ...
            progression = pd.DataFrame({
                "Model": ["Approve All", "Rules", "LR", "LightGBM (raw)", "LightGBM (cal)", "Sentinel v2"],
        ...
        with col_r:
            ...
            comparison = pd.DataFrame({
                "Metric": ["PR-AUC", "Precision", "Recall", "Savings", "Features"],
                "Sparkov": ["0.9513", "94.8%", "95.6%", "94.7%", "15"],
                "IEEE-CIS": ["0.6240", "92.2%", "62.9%", "75.4%", "150"],
            })
            st.table(comparison)

            st.markdown("""
            **Same API, same cost engine, two different datasets.**
            Sparkov is simulated (explainable). IEEE-CIS is real anonymized data.
            Both prove the cost-aware architecture works.
            """)

        st.markdown("---")
        st.markdown("##### Architecture")
        st.markdown("""
        **Endpoints:** `/v1/score` (Sparkov) • `/v1/score/ieee` (IEEE-CIS) • `/health` • `/ready`
        ...
        **Limitations:**
        - Sparkov is simulated — real fraud patterns may differ
        - Velocity features need history — new cards score with defaults
        - Cost parameters are estimates — should be tuned per merchant
